...
precision highp float; in float v_value; flat in float v_state; uniform sampler2D u_palette; uniform float u_opacity; uniform float u_mode; out vec4 outColor; void main(){if(v_state>0.5){outColor=vec4(0.0);return;}float t=clamp((v_value+20.0)/90.0,0.0,1.0);if(u_mode>0.5)t=smoothstep(0.0,1.0,t);if(u_mode>1.5)t=smoothstep(0.08,0.92,t);outColor=texture(u_palette,vec2(t,0.5));outColor.a*=u_opacity;}`;
