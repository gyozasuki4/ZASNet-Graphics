...
export function EditorCanvas({ scene, selectedId, mode, tool, canvasZoom, radarRef, guides, onCanvasZoom, onToolChange, onSelect, onPatch, onCameraChange, onStatus, onDiagnostic, onDropObject, onContextMenu }: Props) {
  ...
  function pointerDown(event: ReactPointerEvent, object: SceneObject, resize?: ResizeDirection) {
    if (clean || object.locked || tool === 'pan-canvas' || spaceHeld) return;
  ...
  return <div className={`canvas-workspace ${clean ? 'clean-output' : ''} ${tool === 'map-nav' ? 'map-navigation-mode' : ''}`} onWheel={wheel} onDrop={drop} onDragOver={event => event.preventDefault()} onContextMenu={event => onContextMenu(event)} onPointerDown={event => { if (mode === 'editor' && (tool === 'pan-canvas' || spaceHeld)) beginPan(event); if (mode === 'editor' && !panDrag && tool === 'select' && event.target === event.currentTarget) onSelect(undefined); }}>
    ...
    <div className="canvas-stage-wrap" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${clean ? 1 : canvasZoom})` }}>
      <div ref={stage} className="scene-stage" style={{ aspectRatio: `${scene.width}/${scene.height}`, background: scene.background }} onDoubleClick={event => { const map = (event.target as HTMLElement).closest('.object-map'); if (map && selectedId && scene.objects.find(item => item.id === selectedId)?.type === 'map') onToolChange('map-nav'); }} onContextMenu={event => onContextMenu(event)} onPointerDown={event => { event.stopPropagation(); if (event.target === event.currentTarget && mode === 'editor') { if (tool === 'map-nav') onToolChange('select'); else onSelect(undefined); } }}>
        ...
        {[...scene.objects].sort((a, b) => a.z - b.z).map(object => object.visible && <div key={object.id} className={`scene-object object-${object.type} ${selectedId === object.id ? 'selected-object' : ''}`} style={{ left: `${object.x / scene.width * 100}%`, top: `${object.y / scene.height * 100}%`, width: `${object.width / scene.width * 100}%`, height: `${object.height / scene.height * 100}%`, opacity: object.opacity, transform: `rotate(${object.rotation}deg)`, zIndex: object.z }} onPointerDown={event => pointerDown(event, object)} onContextMenu={event => onContextMenu(event, object)}>
