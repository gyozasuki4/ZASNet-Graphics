...
export function EditorCanvas({ scene, selectedId, mode, tool, canvasZoom, radarRef, guides, onCanvasZoom, onToolChange, onSelect, onPatch, onCameraChange, onStatus, onDiagnostic, onDropObject, onContextMenu }: Props) {
  ...
  return <div className={`canvas-workspace ${clean ? 'clean-output' : ''} ${tool === 'map-nav' ? 'map-navigation-mode' : ''}`} onWheel={wheel} onDrop={drop} onDragOver={event => event.preventDefault()} onContextMenu={event => onContextMenu(event)} onPointerDown={event => { if (mode === 'editor' && (tool === 'pan-canvas' || spaceHeld)) beginPan(event); if (mode === 'editor' && !panDrag && tool === 'select' && event.target === event.currentTarget) onSelect(undefined); }}>
    ...
    <div className="canvas-stage-wrap" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${clean ? 1 : canvasZoom})` }}>
      <div ref={stage} className="scene-stage" style={{ aspectRatio: `${scene.width}/${scene.height}`, background: scene.background }} onDoubleClick={event => { const map = (event.target as HTMLElement).closest('.object-map'); const object = scene.objects.find(item => item.id === selectedId); if (map) { const mapObject = scene.objects.find(item => item.type === 'map' && (map as HTMLElement).contains(event.target as Node)); if (mapObject) onSelect(mapObject.id); if (mapObject || object?.type === 'map') onToolChange('map-nav'); } }} onContextMenu={event => onContextMenu(event)} onPointerDown={event => { event.stopPropagation(); if (event.target === event.currentTarget && mode === 'editor') { if (tool === 'map-nav') onToolChange('select'); else onSelect(undefined); } }}>
        ...
        {[...scene.objects].sort((a, b) => a.z - b.z).map(object => object.visible && <div key={object.id} className={`scene-object object-${object.type} object-role-${object.role} ${selectedId === object.id ? 'selected-object' : ''}`} style={{ left: `${object.x / scene.width * 100}%`, top: `${object.y / scene.height * 100}%`, width: `${object.width / scene.width * 100}%`, height: `${object.height / scene.height * 100}%`, opacity: object.opacity, transform: `rotate(${object.rotation}deg)`, zIndex: object.z }} onPointerDown={event => pointerDown(event, object)} onContextMenu={event => onContextMenu(event, object)}>
          {object.type === 'map' && <><RadarMapObject ref={radarRef} config={object.properties as unknown as MapObjectProperties} navigation={tool === 'map-nav' && selectedId === object.id} onCameraChange={camera => onCameraChange(object.id, camera)} onStatus={onStatus} onDiagnostic={onDiagnostic} />{tool === 'map-nav' && selectedId === object.id && <div className="map-nav-banner">MAP NAVIGATION · drag to pan · wheel to zoom · Esc to exit</div>}</>}
          {object.type === 'text' && <div className="scene-text" style={{ color: String(object.properties.color), fontFamily: String(object.properties.fontFamily), fontSize: `${Number(object.properties.fontSize) * (100 / 1920)}vw`, fontWeight: Number(object.properties.fontWeight), fontStyle: object.properties.italic ? 'italic' : 'normal', textAlign: object.properties.align as 'left' | 'center' | 'right', WebkitTextStroke: `${Number(object.properties.outlineWidth || 0) * (100 / 1920)}vw ${String(object.properties.outline || '#000')}`, textShadow: object.properties.shadow ? '2px 2px 4px #000b' : undefined }}>{String(object.properties.text)}</div>}
