...
export function RadarMapObject({ config, editor = true }: Props) {
  ...
  useEffect(() => { if (!mapEl.current) return; const map = createMap(mapEl.current); mapRef.current = map; const layer = new RadarLayer(palette); layerRef.current = layer; map.on('load', () => map.addLayer(layer)); return () => map.remove(); }, []);
