...
export const RadarMapObject = forwardRef<RadarMapHandle, Props>(function RadarMapObject({ config, navigation = false, cameraControlled = false, onCameraChange, onStatus, onDiagnostic }, ref) {
  ...
  function applyMapLayers(map = mapRef.current) { if (!map || !map.isStyleLoaded()) return; const cfg = configRef.current; applyMapLayerVisibility(map, { states: cfg.states, counties: cfg.counties, roads: cfg.roads, labels: cfg.labels }); if (map.getLayer('county-highlight')) { map.setFilter('county-highlight', cfg.highlightCounty ? ['==', ['get', 'GEO_ID'], cfg.highlightCounty] : ['==', ['get', 'GEO_ID'], '']); map.setPaintProperty('county-highlight', 'fill-color', cfg.highlightCountyColor || '#45c878'); map.setPaintProperty('county-highlight', 'fill-opacity', cfg.highlightCountyOpacity ?? .35); } const styles = cfg.layerStyles || {}; const lineIds: Record<string, string> = { states: 'state-boundaries', counties: 'county-boundaries', roads: 'roads' }; for (const [key, id] of Object.entries(lineIds)) { const style = styles[key as keyof typeof styles]; if (!style || !map.getLayer(id)) continue; map.setPaintProperty(id, 'line-color', style.color); map.setPaintProperty(id, 'line-opacity', style.opacity); map.setPaintProperty(id, 'line-width', style.width); } const labels = styles.labels; if (labels && map.getLayer('city-labels')) { map.setPaintProperty('city-labels', 'text-color', labels.color); map.setPaintProperty('city-labels', 'text-opacity', labels.opacity); if (labels.haloColor) map.setPaintProperty('city-labels', 'text-halo-color', labels.haloColor); if (labels.textSize) map.setLayoutProperty('city-labels', 'text-size', labels.textSize); } }
  ...
  useEffect(() => { layerRef.current?.setPalette(palette); layerRef.current?.setOpacity(config.opacity ?? 0.85); layerRef.current?.setMode(config.renderMode || 'Broadcast'); layerRef.current?.setVisible(config.radar !== false); mapRef.current?.triggerRepaint(); }, [palette, config.opacity, config.radar, config.renderMode]);
  useEffect(() => { applyMapLayers(); }, [config.states, config.counties, config.roads, config.labels, config.layerStyles, config.highlightCounty, config.highlightCountyColor, config.highlightCountyOpacity]);
  useEffect(() => {
